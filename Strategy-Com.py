...
class StrategyShell:
    ...
    def do_query(self, cur, user, sql):
        try:
            ...
            self.logger.info('strategy_com', sql, 'SUCCESS', 'query', user)
        except Exception as e:
            self.logger.warn('strategy_com', sql, 'FAILED', 'query', user)
            print '[EXCEPTION] -- ',
            print e
    ...
    def do_modify(self, cur, user, sql):
        try:
            ...
            self.logger.info('strategy_com', sql, 'SUCCESS', 'modify', user)
        except Exception as e:
            self.logger.warn('strategy_com', sql, 'FAILED', 'modify', user)
            print '[EXCEPTION] -- ',
            print e
    ...
    def execute(self, user, sql):
        ...
        try:
            ...
            if cmdname == 'select':
                ...
                self.logger.info('strategy_com', sql, 'FAILED', 'unknown', user)
                print '[EXCEPTION] -- unsupported operation -- %s' % sql
        except Exception as e:
            self.logger.warn('strategy_com', sql, 'FAILED', 'unknown', user)
            print '[EXCEPTION] -- ',
            print e
        finally:
            if cur:
