...
_defaultRuledbDB = 'strategy_ebuy'
...
def TakeAction(action):
    ...
    _log.info(_platform, _quest, 'LOG', 'SENSITIVE_EBUY', _username)

    if not level or level == 'RECORD':
        _log.info(_platform, _quest, level, 'SENSITIVE_EBUY', _username)
        canGoOn = True
        return canGoOn
    ...
    if level == 'ALERT':
        _log.warn(_platform, _quest, level, 'SENSITIVE_EBUY', _username)
        canGoOn = True
    else:
        _log.error(_platform, _quest, level, 'SENSITIVE_EBUY', _username)
        canGoOn = False
    return canGoOn
