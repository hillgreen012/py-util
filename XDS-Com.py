...
def TakeAction(action):
    ...
    _log.info(_platform, _quest, 'LOG', 'SENSITIVE_COM', _username)

    if not level or level == 'RECORD':
        _log.info(_platform, _quest, level, 'SENSITIVE_COM', _username)
        canGoOn = True
        return canGoOn
    ...
    if level == 'ALERT':
        _log.warn(_platform, _quest, level, 'SENSITIVE_COM', _username)
        canGoOn = True
    else:
        _log.error(_platform, _quest, level, 'SENSITIVE_COM', _username)
        canGoOn = False
    return canGoOn
